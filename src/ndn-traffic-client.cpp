...
int pil = 1, lamda = 50, nprefix = 0, total_percentage=0;
float zipffactor = 1.75, qvalue = 0; 

void pemilihan_mode(int x){
  pil = x;
}

void nilai_lamda(int x){
  lamda = x;
}

void nilai_q(float x){
  qvalue = x;
}

void faktor_zipf(float x){
  zipffactor = x;
}
...
private:
  ...
  public:
    ...
    parseConfigurationLine(const std::string& line, Logger& logger, int lineNumber)
    {
      ...
      if (parameter == "TrafficPercentage") {
        int x = std::stoul(value);
        m_trafficPercentage = x;
        total_percentage += x;
      }
  ...
  logStatistics()
  {
    ...
    m_logger.log("Total percentage = " + to_string(total_percentage), false, true);
    m_logger.log("Total prefix = " + to_string(nprefix), false, true);
  }
  ...
  generateTraffic(boost::asio::deadline_timer& timer)
  {
    ...
    if (pil == 2){
      static std::poisson_distribution<> trafficDistPoisson(lamda);
      trafficKey = trafficDistPoisson(random::getRandomNumberEngine());
    }
...
main(int argc, char* argv[])
{
  ...
  visibleOptions.add_options()
    ...
    ("mode,m",      po::value<int>(), "(int) Distribution choice : 1. Uniform, 2. Poisson, 3. Zipf-Mandelbrot; Default = Uniform")
    ("lamda,l",     po::value<int>(), "(int) Used in Poisson as lamda, default = 50")
    ("zipffactor,z",po::value<float>(), "(float) Used in Zipf-Mandelbrot as s value, default = 1.75")
